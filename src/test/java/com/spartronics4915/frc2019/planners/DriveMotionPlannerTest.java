...
public class DriveMotionPlannerTest
{
    ...
    @Test
    public void testTwoPoseSCurve()
    {
        DriveMotionPlanner motion_planner = new DriveMotionPlanner();
        System.out.println("testTwoPoseSCurve begin {");
        motion_planner.setFollowerType(DriveMotionPlanner.FollowerType.FEEDFORWARD_ONLY);
        motion_planner.setTrajectory(new TrajectoryIterator<>(new TimedView<>(
                motion_planner.generateTrajectory(false,
                        Arrays.asList(
                                new Pose2d(new Translation2d(0.0, 0.0), Rotation2d.identity()),
                                new Pose2d(new Translation2d(200.0, 0), Rotation2d.fromDegrees(45))),
                        null, // no constraints
                        120.0, 120.0, 10.0)))); // max vel, accel and volt
        double t = 0.0;
        Pose2d pose = motion_planner.setpoint().state().getPose();
        ...
        System.out.println("testTwoPoseSCurve end }");
    }
    ...
    public void testForwardReverseSame()
    {
        ...
        while (!fwd_motion_planner.isDone() || !rev_motion_planner.isDone())
        {
            ...
            Twist2d fwd_delta = Kinematics.forwardKinematics(fwd_output.left_velocity * dt * Constants.kDriveWheelDiameterInches / 2.0,
            ...
            Twist2d rev_delta = Kinematics.forwardKinematics(rev_output.left_velocity * dt * Constants.kDriveWheelDiameterInches / 2.0,
